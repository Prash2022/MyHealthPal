...
public class RegisterActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        edUsername = findViewById(R.id.editTextLTBAddress);
        edPassword = findViewById(R.id.editTextLTBPincode);
        edEmail = findViewById(R.id.editTextLTBFullName);
                ...
                if (username.length()==0 || password.length() ==0 || confirm.length() == 0 || email.length() == 0)
                {
                    ...
                    if (password.compareTo(confirm)==0) {
                        if(isValid(password)) {
                            ...
                            Toast.makeText(getApplicationContext(), "Password must contain atleast 8 characters, having letter, digit and special character!", Toast.LENGTH_SHORT).show();
                        }
                    } else {
